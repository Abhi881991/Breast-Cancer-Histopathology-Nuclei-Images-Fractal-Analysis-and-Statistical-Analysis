...
// Find the correct class name for the command
def commandInfo = commandService.getCommand("at.csa.csaj.plugin2d.frac.Csaj2DFracDimMinkowskiCmd")
if (commandInfo == null) {
    ...
    def possibleCommands = allCommands.findAll { it.getClassName().toLowerCase().contains("minkowski") && it.getClassName().toLowerCase().contains("dim") }
    ...
    if (possibleCommands.size() > 0) {
        ...
        println("ERROR: Could not find Minkowski dimension command class.")
...
resultsTable.appendColumn("# Dilations")   
resultsTable.appendColumn("Reg Start")
resultsTable.appendColumn("Reg End")
resultsTable.appendColumn("Shape type")
resultsTable.appendColumn("Morphological operator")
resultsTable.appendColumn("Dm") 
...
imageFiles.eachWithIndex { file, index ->
    ...
    try {
        ...
        // Configure parameters for the Minkowski dimension calculation
        HashMap<String, Object> parameters = new HashMap<String, Object>()
        parameters.put("datasetIn", datasetIn)
		
		parameters.put("spinnerInteger_NumDilations", "10") //to be adaped
		parameters.put("spinnerInteger_NumRegStart", "1") //to be adapted
		parameters.put("spinnerInteger_NumRegEnd", "10") //to be adapted
		parameters.put("choiceRadioButt_ShapeType", "Disk")
		parameters.put("choiceRadioButt_MorphologicalOperator", "Binary dilation")    
	    parameters.put("booleanShowDoubleLogPlot", "false")
	    parameters.put("booleanShowLastMorphImg", "false")	   
...
if (resultsTable.getRowCount() > 0) {
    uiService.show("2D FracDimMinkowski results", resultsTable)
    
    // Save the results as CSV file
    try {
        def outputFile = new File(inputDir, "minkowski_dimension_results.csv")
